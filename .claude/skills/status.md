...
```bash
# .env exists?
test -f .env && echo "OK .env configured" || echo "X .env missing (run make init)"

# Dependencies installed?
test -d vendor && echo "OK Composer installed" || echo "X Run make composer-install"
test -d node_modules && echo "OK pnpm installed" || echo "X Run make pnpm-install"
...
OK Containers running (5/5)
OK All services healthy
X  Uncommitted changes (3 files)
```
