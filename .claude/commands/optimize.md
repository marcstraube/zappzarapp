...
context: fork
allowed-tools: Read, Write, Edit, Grep, Glob, Bash(date:*), Bash(git:*), Bash(wc:*),
  Bash(ls:*), Bash(jq:*)
argument-hint: [--config | --learnings | --commands | --sessions | --settings | --all]
---
...
**Goal**: Implement GOSS testing
**Duration**: 4 hours
**Key Outcomes**:
- Added container tests
- Fixed nginx config
**Learnings**: 3 (synced to LEARNINGS.md)
...
1. **Never delete without archiving** - All removed content goes to `.claude/archive/`
2. **User approval required** - No automatic modifications to CLAUDE.md or LEARNINGS.md
