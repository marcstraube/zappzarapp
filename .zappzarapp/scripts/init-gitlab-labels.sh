...
# Get current repo
REPO=$(glab repo view --output json 2>/dev/null | jq -r '.path_with_namespace' 2>/dev/null)
if [ -z "$REPO" ] || [ "$REPO" = "null" ]; then
    echo "Error: Not in a GitLab repository"
    exit 1
fi
...
create_label() {
    ...
    if glab label list 2>/dev/null | grep -qw "$name"; then
        echo "Exists:   $name"
    else
        echo "Creating: $name"
...
echo ""
echo "=== Effort Labels (scoped with ::, optional) ==="
