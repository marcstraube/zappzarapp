...
Major introduction of Claude Code multi-agent workflow system and comprehensive
refactoring of AI tool configuration.

#### New: Multi-Agent Workflow System

Complete agent-based workflow for complex tasks in `.claude/agents/`:

- **Agent A (Architect)** — Analysis, plan creation in `.claude/temp/`
- **Agent B1/B2/B3 (Coder)** — Implementation (PHP/Node/SQL)
- **Agent C1-C5 (Reviewer)** — Code review (PHP/Node/SQL/Markdown/ConfigSync)
- **Agent D (Documenter)** — Documentation updates

Supporting files:

- `workflow.md` — Scope decision, pre-flight checks, feature-branch workflow
- `prompts.md` — Agent prompt templates for Task tool
- `.claude/temp/` — Agent work files (plans, drafts)
- `.claude/state/` — Persistent state (audit progress)
- `.claude/cache/` — Temporary data

#### New: Slash Commands

- `/status` — Project overview (Git, Docker, backlog)
- `/commit` — Guided commit workflow with quality checks
- `/backlog` — Manage backlog tasks with 3 targets
- `/learnings` — View and manage project learnings
- `/sync-check` — Check configuration file synchronization
- `/optimize` — Self-optimization of config and commands

#### New: Coding Standards

Language-specific rules in `.zappzarapp/standards/`:

- `php.md` — PHP-CS-Fixer, PHPStan, PHPMD suppressions
- `node.md` — ESLint, Prettier, TypeScript rules
- `sql.md` — MariaDB/PostgreSQL dialect, sqlfluff
- `markdown.md` — Markdownlint rules
- `docker.md` — Hadolint rules
- `make-targets.md` — All lint/test/fix targets

#### 3-Layer Knowledge Architecture

- **Personal** (`~/.local/share/zappzarapp/`) — Private cross-project notes
...
- **Moved** `documentation/` → `.zappzarapp/docs/` (boilerplate docs)
- **New** `.zappzarapp/docs/development/AI-INTEGRATION.md` — Comprehensive AI
  tools documentation (Claude, Gemini, Cursor, Copilot, Cline, Roo Code)
- **New** `.zappzarapp/ai/` — Boilerplate knowledge files
- **New** `.zappzarapp/standards/` — Language-specific coding standards (php,
  node, sql, markdown, docker, make-targets)
...
- **README.md** — Now replaced (not moved) to avoid broken relative paths
- **CLAUDE.md** — Still moved to `.zappzarapp/CLAUDE.md` for reference
- **`.ai/`** — Created automatically for project AI knowledge
- **AI sync hint** — Setup shows `make ai-sync` suggestion for other AI tools
...
- **Folder structures** — Alphabetically sorted in README.md, ARCHITECTURE.md,
  QUICKSTART.md, CLAUDE.md, CLAUDE.template.md
- **Node.js structure** — Fixed `src/node/App/` → `src/node/backend/`,
  `src/node/frontend/`
- **CONTRIBUTING.md** — Added `.gemini/` to personal config list, slimmed AI
  section to reference AI-INTEGRATION.md
