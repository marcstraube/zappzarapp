...
use DevToolbar\Config\Filter\HexColorFilter;
...
final readonly class CookieConfigSource implements MiniBarConfigSource
{
    ...
    private function parseBranchColors(?string $raw): array
    {
        ...
        foreach ($decoded as $key => $value) {
            if (!is_string($key) || !array_key_exists($key, MiniBarConfig::DEFAULT_BRANCH_COLORS)) {
                continue;
            }
            if (!is_string($value) || !$this->colorFilter->isSafe($value)) {
                continue;
            }
    ...
    private function parseThresholds(?string $raw): ?array
    {
        ...
        $validated = [];
        foreach ($decoded as $key => $value) {
            if (!is_string($key) || !in_array($key, self::ALLOWED_THRESHOLD_KEYS, true)) {
                continue;
            }
