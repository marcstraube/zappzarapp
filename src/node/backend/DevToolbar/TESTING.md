...
The DevToolbar uses styled modal dialogs instead of browser `alert()` calls. This guide explains how to test them manually.
...
4. **Expected:** Red error modal with title "Export Failed" and message "No request data available for export."

---

### 2. Test Error Modal - "Request Not Found"

**What it tests:** Error dialog when a historical request was deleted from storage
...
7. **Expected:** Red error modal with title "Export Failed" and message "Request not found in history."
...
3. **Expected:** Browser downloads a JSON file named `devtoolbar-request-{id}-{timestamp}.json`
...
**What it tests:** Modal interaction features (ESC key, overlay click, close button)

**Steps:**
1. Trigger any error modal (use Test 1 or Test 2)
2. **Test ESC key:** Press `Esc` key → Modal should close
3. Trigger the modal again
4. **Test overlay click:** Click outside the modal (on the dark overlay) → Modal should close
5. Trigger the modal again
6. **Test close button:** Click the `×` button in the top-right → Modal should close
...
**Notes:**
- Shortcuts work with any combination of modifiers (Ctrl, Shift, Alt, Cmd/Win) + a key
