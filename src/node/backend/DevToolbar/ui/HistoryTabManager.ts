...
export class HistoryTabManager {
    ...
    private attachFilterListeners(): void {
        const methodFilter = document.getElementById('history-filter-method') as HTMLSelectElement;
        const statusFilter = document.getElementById('history-filter-status') as HTMLSelectElement;
        const uriFilter = document.getElementById('history-filter-uri') as HTMLInputElement;
        const minTimeFilter = document.getElementById('history-filter-min-time') as HTMLInputElement;
    ...
    private filterRequests(): void {
        const filters = {
            method: (document.getElementById('history-filter-method') as HTMLSelectElement)?.value || '',
            status: (document.getElementById('history-filter-status') as HTMLSelectElement)?.value || '',
            uri: (document.getElementById('history-filter-uri') as HTMLInputElement)?.value.toLowerCase() || '',
            minTime: parseFloat((document.getElementById('history-filter-min-time') as HTMLInputElement)?.value || '0'),
        };
    ...
    private resetFilters(): void {
        ['method', 'status', 'uri', 'min-time'].forEach((id) => {
            const el = document.getElementById(`history-filter-${id}`) as HTMLInputElement | HTMLSelectElement;
            if (el) el.value = '';
        });
        this.filterRequests();
    }
